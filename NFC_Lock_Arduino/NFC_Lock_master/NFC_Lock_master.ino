  ...
  if (success) {
    ...
    for (i = 0; i < sizeof(userInfo); i++) {
      ...
      if (responseDigit == 209 || responseDigit == 210){
          ...
          NFCPRINT.println("Found D1 : "); NFCPRINT.println(responseDigit, HEX);
        #endif
    ...
    if (programming_mode == true){
      //TODO: save the card number to EEPROM.
      // creditCardNumber -- > Credit card # of the tapped card
      // size of ^ -- > sizeof(creditCardNumber)

      //writeEEPROM(creditCardNumber,sizeof(creditCardNumber));
      programming_mode=false;
    }
    else{
      //TODO: READ EEPORM or just compare
